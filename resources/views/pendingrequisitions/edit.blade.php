...
<div class="flex justify-end mb-4">
    <button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded shadow" onclick="toggleModal('modal-id')">Action</button>
</div>

<!-- Modal -->
<div id="modal-id" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl overflow-auto max-h-[90vh]">
        
        <!-- Header -->
        <div class="flex justify-between items-center p-4 border-b">
            <h2 class="text-lg font-bold text-gray-800">Requisition Remarks & Approval</h2>
            <button class="text-sm bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded" onclick="toggleModal('modal-id')">Close</button>
        </div>
        ...
        <div class="p-6 space-y-6">
                ...
                <div class="overflow-auto rounded border">
                    <table class="min-w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-4 py-2">{{ $remark->created_at }}</td>
                                <td class="px-4 py-2">{{ $remark->content }}</td>
                                <td class="px-4 py-2">{{ $remark->user->first_name }} {{ $remark->user->last_name }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <!-- Form -->
            <form method="POST" action="{{ route('pendingrequisitions.update.forapproval', $requisition->id) }}" onsubmit="return validateModalForm();">
                @csrf
                @method('PUT')

                <!-- Content Input -->
                <div>
                    <x-input-label for="content" :value="__('Remarks')" />
                    <x-text-input id="content" name="content" type="text" required class="w-full mt-1" :value="old('content', $requisition->content)" />
                    <x-input-error :messages="$errors->get('content')" class="mt-2" />
                </div>

                <!-- Conditional Approval Dropdowns -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <!-- Bonds Status -->
                    <div>
                        <x-input-label for="bonds_status_modal" :value="__('Bonds Approval')" />
                        <select name="bonds_status_modal" class="w-full rounded border-gray-300">
                            @foreach (['no', 'for approval', 'approved', 'return'] as $option)
                                <option value="{{ $option }}" {{ $option === $requisition->bonds_status ? 'selected' : '' }}>
                                    {{ ucfirst($option) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- UW Status -->
                    <div>
                        <x-input-label for="uw_status_modal" :value="__('UW Approval')" />
                        <select name="uw_status_modal" class="w-full rounded border-gray-300">
                            @foreach (['no', 'for approval', 'approved', 'return'] as $option)
                                <option value="{{ $option }}" {{ $option === $requisition->uw_status ? 'selected' : '' }}>
                                    {{ ucfirst($option) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Final Approval -->
                    <div>
                        <x-input-label for="finalapproval_status_modal" :value="__('Final Approval')" />
                        <select name="finalapproval_status_modal" class="w-full rounded border-gray-300">
                            @foreach (['no', 'for approval', 'return'] as $option)
                                <option value="{{ $option }}" {{ $option === $requisition->finalapproval_status ? 'selected' : '' }}>
                                    {{ ucfirst($option) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6 flex flex-col sm:flex-row justify-between items-center gap-3">
                    ...
                    <a href="{{ route('pendingrequisitions.update.forcancel', $requisition->id) }}" onclick="return confirm('Are you sure you want to cancel this requisition?');"
